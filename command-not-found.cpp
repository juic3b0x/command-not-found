...
#ifndef __NEOTERM_PREFIX__
#error "__NEOTERM_PREFIX__ not defined"
#endif

const std::list<std::string_view> main_commands = {
#ifdef __aarch64__
#include "commands-aarch64-neoterm-main.h"
#elif defined __arm__
#include "commands-arm-neoterm-main.h"
#elif defined __i686__
#include "commands-i686-neoterm-main.h"
#elif defined __x86_64__
#include "commands-x86_64-neoterm-main.h"
#else
...
#ifdef __aarch64__
#include "commands-aarch64-neoterm-root.h"
#elif defined __arm__
#include "commands-arm-neoterm-root.h"
#elif defined __i686__
#include "commands-i686-neoterm-root.h"
#elif defined __x86_64__
#include "commands-x86_64-neoterm-root.h"
#else
#error Failed to detect arch
#endif
};
...
#ifdef __aarch64__
#include "commands-aarch64-neoterm-x11.h"
#elif defined __arm__
#include "commands-arm-neoterm-x11.h"
#elif defined __i686__
#include "commands-i686-neoterm-x11.h"
#elif defined __x86_64__
#include "commands-x86_64-neoterm-x11.h"
#else
#error Failed to detect arch
#endif
};
...
inline int neoterm_min3(int a, int b, int c) {
  return (a < b ? (a < c ? a : c) : (b < c ? b : c));
}

int neoterm_levenshtein_distance(char const *s1, char const *s2) {
  int s1len = strlen(s1);
  int s2len = strlen(s2);
  int x, y;
  int **matrix;
  ...
  for (x = 1; x <= s2len; x++) {
    for (y = 1; y <= s1len; y++) {
      matrix[x][y] =
          neoterm_min3(matrix[x - 1][y] + 1, matrix[x][y - 1] + 1,
...
int neoterm_look_for_packages(const char *command_not_found,
  ...
  for (auto it = cmds.begin(); it != cmds.end(); ++it) {
    ...
    if (current_line[0] != ' ') {
      ...
      distance = neoterm_levenshtein_distance(command_not_found,
...
int main(int argc, const char *argv[]) {
  ...
  std::string_view sources_prefix =
      __NEOTERM_PREFIX__ "/etc/apt/sources.list.d/";

  res = neoterm_look_for_packages(command, main_commands, &best_distance,
                                 package_map, "");
  if (res != 0) {
    return res;
  }

  res = neoterm_look_for_packages(command, root_commands, &best_distance,
                                 package_map, "root");
  if (res != 0) {
    return res;
  }

  res = neoterm_look_for_packages(command, x11_commands, &best_distance,
                                 package_map, "x11");
  if (res != 0) {
    return res;
  }
